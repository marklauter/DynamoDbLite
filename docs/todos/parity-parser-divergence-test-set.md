...
The broader inventory in [[docs/notes/parity-coverage-gaps-in-operation-variants.md]] carries the rest: `ConsistentRead`, `ReturnConsumedCapacity`, legacy `ScanFilter` / `AttributesToGet`, multi-OR filter clauses, nested map filtering. Those are real gaps, lower-risk than the parser surface.
