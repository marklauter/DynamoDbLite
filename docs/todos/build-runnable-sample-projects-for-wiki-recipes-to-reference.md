---
title: Build runnable sample projects for wiki recipes to reference
type: todo
summary: "Wiki recipes carry their code as inline blocks that drift from the source; runnable sample projects become the source of truth and the recipes link to them."
tags: [wiki, recipes, samples]
created: 2026-05-16
priority: low
status: open
---
...
2. **No live verification.** A reader who copy-pastes a snippet has no signal that the snippet ran end-to-end on a current build. Nothing checks it before the reader's compiler does.
...
- A recipe links to the sample (`samples/DynamoDbContextForTests/`) for the reader who wants to clone-and-run; the wiki prose explains the *why* and the wrinkles.
...
2. **Build one sample first** — `DynamoDbContextForTests` is the highest-value recipe (zero current wiki coverage, undocumented capability). Get the pattern right on one before propagating.
