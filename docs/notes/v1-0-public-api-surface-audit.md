...
The architecture tests in [`ArchitectureTests.cs`](../../tests/DynamoDbLite.Tests/Architecture/ArchitectureTests.cs) already enforce that types in `DynamoDbLite.SqliteStores`, `DynamoDbLite.SqliteStores.Models`, and `DynamoDbLite.Expressions` are not public. That removes one whole class of leak.
