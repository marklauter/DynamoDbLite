...
Create `src/DynamoDbLite.Parity.Benchmarks/` with BenchmarkDotNet. Mirror the `ParityBackend` enum (DdbLite / DdbLiteFile / DynamoDbLocal) as benchmark parameter axis. Coverage to include:
