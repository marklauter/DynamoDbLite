...
- [`ScanParityTests`](../../tests/DynamoDbLite.Parity.Tests/ScanParityTests.cs) — `FilterExpression` with correct `Count` and `ScannedCount`; `contains` on string set; `IN` against a value list; `Limit` paginated to exhaustion via `ExclusiveStartKey`/`LastEvaluatedKey`, with each cursor pinned to exactly the hash key of its page's last item; a `Limit` dividing the table exactly, which ends the walk on an empty page; `Limit` bounding the pre-filter window under a `FilterExpression`; the same pagination walk over a GSI and an LSI, where the cursor carries the index keys alongside the table keys; parallel scan with `Segment`/`TotalSegments` and no `Limit` — `Limit` combined with `TotalSegments` still diverges from real DynamoDB, see `parallel-scan-limit-interaction-gap.md`.
