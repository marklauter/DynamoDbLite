---
title: Parser-result caching
type: note
summary: "Cache parsed expression ASTs keyed by raw expression text; converts per-call parse cost into a one-time cost per distinct expression."
tags: [release, v1.1, performance, parser, bench-needed]
created: 2026-05-15
status: evolving
refines: "[[dynamodblite-write-path-is-slower-than-read-path]]"
...
Cache parser results by raw expression text. The cache key is the expression string; the value is the AST. Cache lookup is O(string-hash); on a hit, parsing cost goes from "tokenize + parse + AST allocations" to a dictionary read. On a miss, the cost is the same as today.

The same expression text reused across many calls (the common case) becomes a single parse for the application's lifetime. The cache has to be bounded. An application can generate an unbounded number of distinct expression strings. A default cap of a few thousand entries with LRU eviction handles realistic workloads.
...
If the AST has any mutable fields, the cache returns a structural template and per-call evaluation binds attribute names/values at evaluate-time. That is already the pattern in `ConditionExpressionEvaluator.Evaluate`.
