...
type: decision
summary: "No operation defers work past its own return. Export and import complete before responding while still reporting IN_PROGRESS for parity; the TTL sweep is awaited on the read path, throttled to once per table per 60 seconds."
tags: [ttl, export, import, concurrency]
created: 2026-07-29
status: locked
supersedes: "[[implementation-phases]]"
---
...
observe is unchanged. Only the timing changed.
...
Moving the TTL sweep onto the read path is safe for a separate reason: reclamation was never
load-bearing. Every read path filters expired rows by `ttl_epoch`, so an expired item is invisible the
moment it expires, whether or not the sweep has run. Correctness does not depend on when the sweep
runs.

## Consequence

Export and import latency now lands on the caller. For the local databases this library targets, that
is milliseconds. That cost buys a terminal status that is true by the time the call returns, so a
caller that polls `DescribeExport` sees `COMPLETED` on its first poll.

The TTL sweep is throttled to once per table per 60 seconds, up from 30. Its cost amortizes across
read traffic: one read per table per minute pays for the sweep, and every other read returns after a
dictionary lookup.

A sweep failure is logged and swallowed, because the caller's operation has already succeeded.
`OperationCanceledException` propagates instead. The sweep shares the caller's token, so a cancelled
sweep means a cancelled request.
...
The accepted trade-off: an export of a large table blocks its caller where real DynamoDB would not.
The blocking duration is bounded by local disk throughput rather than a network round trip.
